...
CREATE TABLE USER_FOREIGN_KEY(
    ...
    -- USER FOREIGN_KEY에 있는 GRADE_CODE는 USER_GRADE의 GRADE_CODE가 가지고 있는10, 20, 30만 넣을 수 있어요
...
DROP TABLE SHOP_BUY;
-- SHOP_BUY의 USER_ID는 SHOP_MEMBER에 USER_ID가 가지고 있는 khuser01, khuser02등..만 넣을 수 있어요.
SELECT * FROM SHOP_BUY;
...
-- 참조 무결성을 보장하는 제약조건임.
-- 외래키 설정방법
-- 컬럼 레벨 : REFERENCES 부모 테이블(컬럼명) 삭제 옵션(ON DELETE SET NULL, ON DELETE CASCADE)
-- 외래키 삭제옵션
