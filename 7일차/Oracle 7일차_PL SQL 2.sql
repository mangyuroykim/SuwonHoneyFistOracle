...
-- 송중기 사원의 사번, 사원명, 직급명을 익명블럭을 통해 출력하세요.
...
-- PL/SQL 입력받기
DECLARE
    VEMID EMPLOYEE.EMP_ID%TYPE;
    VENAME EMPLOYEE.EMP_NAME%TYPE;
    VDCODE EMPLOYEE.DEPT_CODE%TYPE;
    VDTITLE DEPARTMENT.DEPT_TITLE%TYPE;
BEGIN
    -- 쿼리문 작성
    SELECT EMP_ID, EMP_NAME, DEPT_CODEM DEPT_TITLE
    -- 변수에 쿼리문 결과값 매핑
...
SELECT *
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = '&EMP_ID';
    DBMS_OUTPUT.PUT_LINE('사원번호 : ' || VEMP.EMP_ID ||', 이름 : ' || VEMP.EMP_NAME ||', 부서명 : ' );
END;
/


-- @실습문제3
-- EMPLOYEE 테이블은 이름, 주민번호, 전화번호, 직급코드, 급여등급을 등에서 사번의 마지막 번호를 구한뒤 +1한 사번에 사용자로부터
-- 입력받록하는 PL/SQL을 작성하시오.
