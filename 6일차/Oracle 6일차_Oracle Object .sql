...
--  1.2.2 Inline View ( 저장 x ) - FROM절 뒤에 적는 서브 쿼리
...
--  ex) 전체 직원의 사전, 이름, 직급명, 부서명, 지역명을 볼 수 잇는 VIEW를 생성하시오(ALL_INFO_ VIEW)
...
-- 뷰(V_EMP_INFO)를 (읽기 전용으로)
-- 생성하여라.

CREATE OR REPLACE VIEW V_EMP_INFO (사원아이디, 사원명, 직급명, 부서명, 관리자명, 입사일) 
AS
SELECT EMP_ID, EMP_NAME, JOB_NAME, NVL(DEPT_TITLE, '미정'), NVL((SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = E.MANAGER_ID), '없음'), HIRE_DATE
FROM EMPLOYEE E
...
(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = 송중기의 매니저ID);
...
-- 2.2.6 NOCACHE : 메모리 상에서 시퀀스 값을 관리, 기본값 20, NOCACHE를 안하면 갑자기 시퀀스 값이 증가하게 됨
...
-- ID 값은 500번부터 시작하여 10씩 증가하여 저장
-- JOIN_COM값은 1번부터 시작하여 1씩 증가하여 저장
