...
SELECT EMP_NAME AS 이름, HIRE_DATE "입사일", SYSDATE AS "오늘", ROUND(SYSDATE - HIRE_DATE) "근무일수"
FROM EMPLOYEE;
--    (SYSDATE를 사용하면 현재 시간 출력)
...
-- 문제2.
-- 재직중이 아닌 직원의 이름,부서코드, 고용일, 근무기간,
-- 퇴직일을 검색하여라 
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME "이름", DEPT_CODE "부서코드", HIRE_DATE "고용일",ENT_YN "퇴사여부"
,(ENT_DATE - HIRE_DATE) AS "근무기간"
...
-- 출력 결과는 이름,급여,근속년수(소수점X)를 
-- 근속년수가 오름차순으로 정렬하여 출력하라
...
-- 문제4.
-- 입사일이 99/01/01 ~ 10/01/01 인 사람 중에서 급여가 
-- 2000000 원 이하인 사람의
...
-- 문제5.
-- 급여가 2000000원 ~ 3000000원 인 여직원 중에서 4월 생일자를 
-- 검색하여 이름,주민번호,급여,부서코드를 주민번호 순으로(내림차순) 출력하여라
...
-- 1000일 마다(소수점 제외) 
-- 급여의 10% 보너스를 계산하여 이름,특별 보너스 (계산 금액) 결과를
-- 출력하여라.
