...
-- 메인 쿼리가 서브 쿼리를 포함하는 종속적인관계
-- 서브 쿼리는 반드시 소괄호로 묶어야 함
...
SELECT DEPT_CODE, EMP_NAME
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID FROM DEPARTMENT JOIN LOCATION ON LOCATION_ID =LOCAL_CODE WHERE LOCAL_NAME = UPPER('Asia1'));
...
SELECT * FROM EMPLOYEE WHERE EMP_ID IN ('200', '201', '204', '207', '211');
...
SELECT 1 FROM EMPLOYEE WHERE SALARY > 3700000;
SELECT 1 FROM EMPLOYEE WHERE SALARY > 2800000;
SELECT 1 FROM EMPLOYEE WHERE SALARY > 3400000;
SELECT 1 FROM EMPLOYEE WHERE SALARY > 3900000;
SELECT 1 FROM EMPLOYEE WHERE SALARY > 1800000;
...
HAVING DEPT_CODE = 'D5')
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'; -- 메인 쿼리에 SELECT 뒤에 월 평균급여 서브쿼리 사용, 3번
...
-- @실습문제4
-- 급이 J1, J2, J3이 아닌 사원중에서 자신의 부서별 평균급여보다 많은 급여를 받는
